...
import { products } from "@/prisma/constant";
...
export const SearchInput: React.FC<Props> = ({ className }) => {
    ...
    useClickAway(ref, () => {
      ...
      useDebounce(
        async () => {
          try {
            const response = await Api.products.search(seacrhQuery);
    ...
    return (
        ...
        <div ref={ref} className={cn('flex rounded-2xl flex-1 justify-between relative h-11 z-30', className)}>
            ...
            {products.length > 0 &&
            <div className={cn(
              'absolute w-full bg-white rounded-xl top-14 pl-2 shadow-md transition-all duration-200 invisible opacity-0 z-30',
